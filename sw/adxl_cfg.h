...
typedef struct {
	uint32_t ctl_reg; // 0x00
	uint32_t request_interval_reg; // 0x04
	uint32_t read_valid_count_reg; // 0x08
	uint32_t write_valid_count_reg; // 0x0C
	uint32_t read_transactions_lsb_reg; // 0x10
	uint32_t read_transactions_msb_reg; // 0x14
	uint32_t write_transactions_lsb_reg; // 0x18
	uint32_t write_transactions_msb_reg; // 0x1C
	uint32_t opt_request_interval_lsb_reg; // 0x20
	uint32_t opt_request_interval_msb_reg; // 0x24
	uint32_t clk_period_reg;
	uint32_t data_width_reg;
	uint32_t single_req_params_reg;
} adxl_cfg;
...
/*Mask for REG_15 - SINGLE_REQUEST parameters*/
#define CFG_SINGLE_REQ_PARAMS_ADDRESS_MASK 		  0x003F0000
#define CFG_SINGLE_REQ_PARAMS_SIZE_MASK 		  0x00003F00
#define CFG_SINGLE_REQ_PARAMS_REQUEST_MASK 		  0x00000001

/*CTL reg defines*/
#define adxl_cfg_ctl_reset(ptr) ((ptr)->ctl_reg |= CFG_CTL_RESET_LOGIC_MASK)
#define adxl_cfg_ctl_reset_completed(ptr) ((ptr)->ctl_reg & CFG_CTL_RESET_LOGIC_MASK) ? FALSE : TRUE
...
#define adxl_cfg_set_single_req_params_address(ptr, data) (ptr)->single_req_params_reg = (((ptr)->single_req_params_reg & ~CFG_SINGLE_REQ_PARAMS_ADDRESS_MASK) + (data & CFG_SINGLE_REQ_PARAMS_ADDRESS_MASK))
...
#define adxl_cfg_set_single_req_params_single_request(ptr) ((ptr)->single_req_params_reg |= CFG_SINGLE_REQ_PARAMS_REQUEST_MASK)
