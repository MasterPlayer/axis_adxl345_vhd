...
int gic_init(XScuGic *ptr, axi_adxl* adxl_ptr){
    ...
    XScuGic_SetPriorityTriggerType(ptr, XPAR_FABRIC_AXI_ADXL345_0_ADXL_IRQ_INTR, 0x00, 0x1);

    status = XScuGic_Connect(ptr, XPAR_FABRIC_AXI_ADXL345_0_ADXL_IRQ_INTR, (Xil_InterruptHandler)adxl_intr_handler, adxl_ptr);
    if (status != XST_SUCCESS){
        return XST_FAILURE;
    }

    XScuGic_Enable(ptr, XPAR_FABRIC_AXI_ADXL345_0_ADXL_IRQ_INTR);
